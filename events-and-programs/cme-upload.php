<?php
/**
 * CME flyer upload API
 * Protects uploads with a simple password (change CME_UPLOAD_PASSWORD below).
 */
header('Content-Type: application/json; charset=utf-8');

const CME_UPLOAD_PASSWORD = 'sschrc-cme-2026'; // change this before going live
...
if (!hash_equals(CME_UPLOAD_PASSWORD, $password)) {
...
$folder = __DIR__ . '/assets/cme/flyers/' . $status;
...
$jsonPath = __DIR__ . '/cme-flyers.json';
...
$entry = [
    ...
    'time' => $time,
    'venue' => $venue,
    'flyer' => 'assets/cme/flyers/' . $status . '/' . $filename,
...
if ($encoded === false || file_put_contents($jsonPath, $encoded . PHP_EOL) === false) {
    respond(false, 'Flyer saved, but failed to update the CME list file.');
}

respond(true, 'Flyer uploaded successfully.', [
    'entry' => $entry,
    'redirect' => 'cme.html#' . $status,
]);
