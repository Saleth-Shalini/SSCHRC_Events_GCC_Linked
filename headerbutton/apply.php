...
// Guard every constant — db_config.php may already define some of these.
// defined() check prevents "Cannot redefine constant" fatal errors.
if (!defined('UPLOAD_DIR'))      define('UPLOAD_DIR',      __DIR__ . '/uploads/cv/');
if (!defined('UPLOAD_URL_PATH')) define('UPLOAD_URL_PATH', 'uploads/cv/');
if (!defined('ALLOWED_EXT'))     define('ALLOWED_EXT',     ['pdf', 'doc', 'docx']);
if (!defined('ALLOWED_MIME'))    define('ALLOWED_MIME',    [
    // PDF
    'application/pdf',
    'application/x-pdf',
    // DOC (old binary Word format — finfo may return any of these)
    'application/msword',
    'application/vnd.ms-word',
    'application/vnd.ms-office',
    'application/x-ole-storage',          // raw OLE container
    // DOCX (ZIP-based — finfo often returns application/zip)
    'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
    'application/zip',
    'application/x-zip-compressed',
    // Generic binary — last-resort fallback for strict finfo builds
    'application/octet-stream',
]);
if (!defined('MAX_FILE_SIZE'))   define('MAX_FILE_SIZE',   2 * 1024 * 1024); // 2 MB
...
} elseif (!preg_match('/^[6-9][0-9]{9}$/', $contact_number)) {
...
// Allow direct download of CV files; block directory listing
$cvHtaccess = "Options -Indexes\n\n"
    ...
    . "    </IfModule>\n"
    . "</FilesMatch>\n";
file_put_contents(UPLOAD_DIR . '.htaccess', $cvHtaccess);
...
if (!move_uploaded_file($_FILES['cv_file']['tmp_name'], $destination)) {
    error_log('[Career CV Upload] move_uploaded_file failed. UPLOAD_DIR=' . UPLOAD_DIR . ' | destination=' . $destination . ' | is_dir=' . (is_dir(UPLOAD_DIR) ? 'yes' : 'no') . ' | is_writable=' . (is_writable(UPLOAD_DIR) ? 'yes' : 'no'));
...
try {
    ...
    // ── Additional sync to external CarrierPageDataInsert API (DB remains primary) ──
    require_once __DIR__ . '/career_api_sync.php';

    $apiDob = $dob
        ? implode('/', array_reverse(explode('-', $dob)))
        : '';

    careerSyncToExternalApi([
        'COM'                => 1,
        'LOC'                => 2,
        'CAREERID'           => '',
        'POSTAPPLIED'        => $post_applied_for,
        'APPLIEDON'          => date('d/m/Y'),
        'TITLE'              => $title,
        ...
        'GENDER'             => $gender,
        'DOB'                => $apiDob,
        'NATIONALITIY'       => $nationality,
        'PHONENO'            => $contact_number,
        'EMAIL'              => $email_id,
        'ADDRESS'            => $address,
        'QUALIFICATION'      => $qualification_1,
        'ATTACHMENTURL'      => careerAttachmentAbsoluteUrl($cv_file_path),
        'EXPERIENCEYEAR'     => $experience_years  ?? 0,
        'EXPERIENCEMONTH'    => $experience_months ?? 0,
        'CURRENTCTC'         => $current_ctc !== null ? (string) $current_ctc : '',
        'EXPECTEDCTC'        => $expected_ctc !== null ? (string) $expected_ctc : '',
        'CURRENTCOMPANY'     => $current_company,
        'NOTICEPERIOD'       => $notice_period !== null ? (string) $notice_period : '',
        'PREFEREDLOCATION'   => $preferred_location,
